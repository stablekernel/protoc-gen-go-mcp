...
// Generate generates an example MCP server.
func Generate() {
	mg.SerialDeps(Build)

	fmt.Println("=> generate")
	env["PATH"] = fmt.Sprintf("%s:%s", fmt.Sprintf("%s/%s", workDir, binDir), os.Getenv("PATH"))
	examplesDir := "examples"
	protosDir := examplesDir + "/protos"

	sh.Rm(examplesDir + "/gen")
	bash("protoc --go_out=. --go-grpc_out=. --go-mcp_out=. --proto_path=%s %s/example.proto", protosDir, protosDir)
}
